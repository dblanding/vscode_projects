...
namespace
{
  ...
  namespace ShapeReflection
  {
    ...
    TopoDS_Shape CreateRoot(const TopoDS_Shape& chassis)
    {
      TopoDS_Compound compShape;
      BRep_Builder bbuilder;
      bbuilder.MakeCompound(compShape);

      bbuilder.Add( compShape, chassis );
...
int main(int argc, char** argv)
{
  ...
  /* Main assembly.
     The STEP file would still contain just a couple of entities.
  ...
  */
  ...
  t_prototype rootPrototype;
  rootPrototype.shape = ::ShapeReflection::CreateRoot(chassisPrototype.shape);
  rootPrototype.label = ST->AddShape(rootPrototype.shape, true);
  ...
  TDataStd_Name::Set(rootPrototype.label,      "root");
   ...
   */
  PCDM_StoreStatus sstatus = app->SaveAs(doc, "/home/doug/Desktop/test.xbf");
  //
  ...
  app->Close(doc); // Close the document.
  return 0;
}
